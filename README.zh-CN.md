...
- **翻译链路加固**——注入前会清洗 LLM 输出，OpenAI 客户端增加超时/重试，`sync` 会汇总失败项
- **文件操作更稳妥**——cache / overrides 改为原子写入，且只在真正写入时创建 translations 目录
- **CLI 体验提升**——新增 `sync --dry-run`，`verify` 会直接检查 zh/ja/ko 的实际 frontmatter 内容
...
也可以使用 `python -m claude_translator` 运行 CLI。

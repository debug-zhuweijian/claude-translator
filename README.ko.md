...
- **재귀 탐색 범위** -- 사용자 agents, 중첩 commands, 중첩 skill bundle 의 `SKILL.md` 파일을 탐색
- **플러그인 중첩 엔트리** -- 플러그인 commands, agents, skill bundles 가 재귀 엔트리를 지원하며 최신 버전 deduplication 을 유지
- **탐색 감사 모드** -- `discover --audit` 가 scope/kind 개수, dogfooding 카테고리, 누락된 frontmatter, 빈 description 을 보고
- **네임스페이스 canonical ID** -- `gsd:add-backlog`, `ce:brainstorm` 같은 command / agent 이름을 올바르게 파싱

### v0.4.0

- **공유 atomic write helper** -- cache, overrides, legacy migration 쓰기 경로가 공통 atomic writer 를 사용
- **클라이언트 coverage 확대** -- 동기/비동기 OpenAI 클라이언트와 공유 언어 helper 테스트 강화
- **릴리스급 CI** -- PR 과 master push 에서 Python 3.10-3.13 matrix 및 93% coverage gate 실행
- **패키징 및 prompt 강화** -- editable metadata 검증을 수정하고 XML prompt tag breakout 을 escape

### v0.3.0

- **비동기 번역 스택** -- 비동기 클라이언트와 `TranslationChain.translate_async` 가 동시 번역 지원
- **제한된 동시성 파이프라인** -- `sync` 가 async mode, concurrency control, rich progress output 지원
- **빈 출력 명확한 집계** -- 빈 모델 출력은 skipped records 와 분리해 보고
- **Prompt 및 쓰기 경로 강화** -- 사용자 텍스트를 prompt 안에서 격리하고 번역 쓰기에 allowed paths 강제

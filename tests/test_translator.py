...
def test_override_hit():
    chain = TranslationChain(
        overrides={"plugin.test.skill:demo": "用户覆盖"},
        cache={},
...
def test_cache_hit():
    chain = TranslationChain(
        overrides={},
        cache={"plugin.test.skill:demo": "缓存翻译"},
        on_cache_update=lambda lang, cid, text: None,
        client=FakeClient(),
        target_lang="zh-CN",
    )
...
def test_override_hit_does_not_build_llm_client():
    built = False
...
def test_client_factory_used_on_first_llm_miss():
    built = 0
    updated: dict[str, str] = {}
    ...
    assert r.matched_translation == "[zh-CN] Hello"
    assert r.status == "llm"
    assert updated["plugin.test.skill:demo"] == "[zh-CN] Hello"
    assert built == 1

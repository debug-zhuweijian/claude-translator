...
| `govern [--lang LANG] [--apply]` | 厳格な description governance を計画または適用。既定は dry-run、適用時は backup manifest を作成 |
...
- **再帰的な検出範囲** -- ユーザー agents、ネストした commands、ネストした skill bundle の `SKILL.md` を検出
- **プラグインのネスト入口** -- プラグイン commands、agents、skill bundles が再帰的な入口に対応し、最新バージョンの重複排除も維持
- **検出監査モード** -- `discover --audit` が scope/kind 件数、dogfooding カテゴリ、frontmatter 欠落、空 description を報告
- **名前空間付き canonical ID** -- `gsd:add-backlog` や `ce:brainstorm` のような command / agent 名を正しく解析

### v0.4.0

- **共有 atomic write helper** -- cache、overrides、legacy migration の書き込みで共通の atomic writer を使用
- **クライアント coverage 拡大** -- 同期/非同期 OpenAI クライアントと共有言語 helper のテストを強化
- **リリース級 CI** -- PR と master push で Python 3.10-3.13 matrix と 93% coverage gate を実行
- **パッケージングと prompt 強化** -- editable metadata 検証を修正し、XML prompt tag breakout をエスケープ

### v0.3.0

- **非同期翻訳スタック** -- 非同期クライアントと `TranslationChain.translate_async` が並行翻訳に対応
- **有界並行パイプライン** -- `sync` が async mode、concurrency control、rich progress output に対応
- **空出力の明確な集計** -- 空のモデル出力を skipped records とは別に報告
- **Prompt と書き込みパスの強化** -- ユーザーテキストを prompt 内で隔離し、翻訳書き込みに allowed paths を強制
